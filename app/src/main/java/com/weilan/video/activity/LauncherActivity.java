...
import com.weilan.video.AppContext;
        ...
        JMLinkAPI.getInstance().register(new JMLinkResponse() {
            ...
            public void response(JMLinkResponseObj obj) {
               // Log.e("JMLinkAPI", "replay = " + obj.paramMap + " " + obj.uri + " " + obj.source + " " + obj.type);
                ...
                String u_id = null;
                String inviteid0 = obj.paramMap.get("u_id");
                String one ;
                if(inviteid0 != null){
                    one = "paramMap = "+inviteid0;
                    u_id = inviteid0;
                }else{
                    one = "paramMap null";
                }
                ...
                if(inviteid != null){
                    one += ",JMLinkAPI = "+inviteid;
                    u_id = inviteid;
                }else{
                    one += ",JMLinkAPI = null";
                }
                if(u_id != null){
                    SpUtil.getInstance().setStringValue(CommonAppContext.JMMSG,u_id);
                }
                //ToastUtil.show(one);
            }
        });
...
//        LanguageUtil.changeLanguage(this, LanguageEnum.VIETNAM);
