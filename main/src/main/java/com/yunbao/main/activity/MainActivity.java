...
import com.yunbao.main.views.MainShareViewHolder;
...
//    private MainActiveViewHolder mActiveViewHolder;
    ...
    private MainShareViewHolder mShareViewHolder;
...
//        boolean showInvite = getIntent().getBooleanExtra(Constants.SHOW_INVITE, false);
        ...
        checkVersion();
        checkIsUploadInvite();
//        String comment = CommonAppConfig.getInstance().getComment();
//        if(!TextUtils.isEmpty(comment)){
//            Log.i("apkinfo", "读取到当前apk含有邀请信息："+comment);
//            sendInvitationCode(comment);
//        }
//        if (showInvite) {
//            showInvitationCode();
//        }
    ...
    //检查是否已上传invitercode
    private void checkIsUploadInvite() {
        Log.i("apkinfo", "checkIsUploadInvite enter");
        ...
        if(TextUtils.isEmpty(inviter)||inviter.equals("")||inviter.equals("uploaded")){
            Log.i("apkinfo", "checkIsUploadInvite 当前已发送 或当前未收到inviter信息");
            return;
        }
        Log.i("apkinfo", "checkIsUploadInvite 发送邀请信息"+inviter);
        sendInvitationCode(inviter);
                        ...
                        if (code == 0 && info.length > 0) {
                            ...
                            if (CommonAppConfig.LIVE_SDK_CHANGED) {
                                LiveAnchorActivity.forward(mContext,
                                        obj.getIntValue("live_sdk"), JSON.parseObject(obj.getString("android"), LiveKsyConfigBean.class), haveStore);
                            } else {
                                LiveAnchorActivity.forward(mContext, CommonAppConfig.LIVE_SDK_USED, LiveConfig.getDefaultKsyConfig(), haveStore);
                            }
                        }
                    }
                };
    ...
    private void sendInvitationCode(String code) {
...
//                    vh = mActiveViewHolder;
                    ...
                    mShareViewHolder = new MainShareViewHolder(mContext,parent);
                    vh = mShareViewHolder;
                } else if (position == 2) {
    ...
    public static final int requestToCharge = 101;
    @Override
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        super.onActivityResult(requestCode, resultCode, data);
        if(requestCode == requestToCharge && resultCode == requestToCharge){
            RouteUtil.forwardMyCoin(this);
        }
    }
}
