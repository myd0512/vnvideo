...
import com.yunbao.common.Language.LanguageEnum;
...
public class MainMeViewHolder extends AbsMainViewHolder implements OnItemClickListener<UserItemBean>, View.OnClickListener {
    ...
    public void onItemClick(UserItemBean bean, int position) {
        ...
        //Log.i("apkinfo", bean.getId()+bean.getName()+bean.getHref()+bean.getThumb());
        if (TextUtils.isEmpty(url)) {
            ...
            if (bean.getId() == 8) {//三级分销
                Log.i("apkinfo", url+"---------"+bean.getName());
    ...
    String[] strings = new String[]{WordUtil.getString(R.string.select_language_vi),
            WordUtil.getString(R.string.select_language_zh),
            WordUtil.getString(R.string.select_language_en)};
    int defIndex = LanguageUtil.getLanguage(mContext);

    private void changeLanguage(){
        new AlertDialog.Builder(mContext).setTitle("选择语言").setSingleChoiceItems(R.array.changeLanguage, defIndex, new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialogInterface, int i) {
                dialogInterface.dismiss();
                if(i == 0){
                    LanguageUtil.changeLanguage(mContext, LanguageEnum.VIETNAM);
                    LanguageUtil.applyLanguage(mContext);
                }else if(i == 1){
                    LanguageUtil.changeLanguage(mContext, LanguageEnum.SIMPLIFIED_CHINESE);
                    LanguageUtil.applyLanguage(mContext);
                }
