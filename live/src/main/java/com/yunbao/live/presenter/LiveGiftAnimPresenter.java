...
//        LogUtil.elong("showGift->","showGiftAnim" + (bean.getGif() == 1));
        if (bean.getGif() == 1) {
            showGifGift(bean);
        } else {
            showNormalGift(bean);
    ...
    private void playHaoHuaGift(File file) {
        //LogUtil.elong("show gift->","playHaoHuaGift");
    ...
    private void showNormalGift(LiveReceiveGiftBean bean) {
        LogUtil.elong("showGift->","showGiftAnim" + bean.getGifUrl());
        if (mLiveGiftViewHolders[0].isIdle()) {
//            LogUtil.elong("showGift->","1");
