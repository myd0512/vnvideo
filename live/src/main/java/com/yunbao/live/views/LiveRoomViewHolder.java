...
public class LiveRoomViewHolder extends AbsViewHolder implements View.OnClickListener {
    ...
    private RecyclerView mChatOtherMsgRecyclerView;//分离其他信息 chat_recyclerView_other
    private LiveUserAdapter mLiveUserAdapter;
    private LiveChatAdapter mLiveChatAdapter;

    private LiveChatAdapter mLiveOtherChatAdapter;
    ...
    public void init() {
        ...
        //其他聊天框
        mChatOtherMsgRecyclerView = (RecyclerView) findViewById(R.id.chat_recyclerView_other);
        mChatOtherMsgRecyclerView.setHasFixedSize(true);
        mChatOtherMsgRecyclerView.setLayoutManager(new LinearLayoutManager(mContext, LinearLayoutManager.VERTICAL, false));
        mChatOtherMsgRecyclerView.addItemDecoration(new TopGradual());
        mLiveOtherChatAdapter = new LiveChatAdapter(mContext);
        mLiveOtherChatAdapter.setOnItemClickListener(new OnItemClickListener<LiveChatBean>() {
            @Override
            public void onItemClick(LiveChatBean bean, int position) {
                showUserDialog(bean.getId());
            }
        });
        mChatOtherMsgRecyclerView.setAdapter(mLiveOtherChatAdapter);
        ...
        findViewById(R.id.btn_live_game).setOnClickListener(this);
    ...
    public void onClick(View v) {
        ...
        }else if (i == R.id.btn_live_game) {
            ((LiveActivity) mContext).openGameBox();
        }
    }

    public void setDiceGameVis(boolean visib){
        findViewById(R.id.btn_live_game).setVisibility(visib ? View.VISIBLE : View.INVISIBLE);
    }
...
//        Log.e("msg-->",bean.getContent()+";;;"+bean.getType());
        if(bean.getType() == 1 || bean.getType() == 3 ){
            if (mLiveOtherChatAdapter != null) {
                mLiveOtherChatAdapter.insertItem(bean);
                return;
            }
        }
